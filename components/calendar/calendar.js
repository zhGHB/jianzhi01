...
Component({
    ...
    methods: {
        goRes(e) {
            ...
            if (num) {
            	wx.navigateTo({
            	 url:`../res/res?date=${this.data.currentYear}/${this.data.currentMonth}/${date}`,
            	})
            }
        },
        // 获取某年某月总共多少天
        ...
        getAllArr() {
            ...
            let params = this.data.currentYear + '/' + this.data.currentMonth + '/1';
            api.calendarList({ ym: params }).then((res) => {
                res.map((Item, index) => {
                    let myDate = Item.start_time.substring(8).replace('0', '');
                    myDate = parseInt(myDate);
                    allArr.map((item, index) => {
                        if (item.month === 'current' && parseInt(item.date) === myDate) {
                            item.id = Item.id;
                            item.num = Item.num;
                        }
                    })
